...
public interface CaseFlagRepository extends JpaRepository<CaseFlagDto, Long> {
    ...
    @Query(value = "with recursive flagdetailscategorypath" +
        "    AS ( " +
        "       SELECT a.id, a.flag_code, a.value_en, a.category_id, a.value_en AS categorypath " +
        "         FROM flag_details a " +
        "        WHERE a.category_id = 0 " +
        "       UNION ALL " +
        "       SELECT b.id, b.flag_code, b.value_en, b.category_id, concat(c.categorypath,'/',b.value_en) AS categorypath " +
        "         FROM flag_details b " +
        ...
        "    AS ( " +
        "       SELECT j.flag_code, j.hearing_relevant, j.request_reason " +
        "         FROM flag_service j " +
        ...
        "       UNION " +
        "       SELECT l.flag_code, l.hearing_relevant, l.request_reason " +
        "         FROM flag_service l " +
        ...
        "     , relevantcategories " +
        "    AS ( " +
        "       SELECT s.id, s.flag_code, s.value_en, s.category_id, s.value_en AS categorypath " +
        "         FROM flag_details s " +
        "        WHERE s.id IN (SELECT t.category_id " +
        "                         FROM distinctcategoryid t) " +
        "       UNION ALL " +
        "       SELECT u.id, u.flag_code, u.value_en, u.category_id, u.value_en AS categorypath " +
        "         FROM flag_details u " +
        "       INNER JOIN relevantcategories v " +
        "               ON u.id = v.category_id " +
        "       ) " +
        "SELECT h.id, h.flag_code, h.value_en, h.category_id, f.categorypath, " +
        "       h.hearing_relevant, h.request_reason, h.isparent " +
        "  FROM flagdetailscategorypath f " +
        "INNER JOIN " +
        "( " +
        " SELECT d.id, d.flag_code, d.value_en, d.category_id, d.categorypath, " +
        "        e.hearing_relevant, e.request_reason, 'FALSE' AS isparent " +
        "   FROM flagdetailscategorypath d " +
        " INNER JOIN flagserviceconsolidated e " +
        "         ON d.flag_code = e.flag_code " +
        " UNION " +
        " SELECT p.id, 'CATGRY' AS flag_code, p.value_en, p.category_id, p.categorypath, " +
        "        'FALSE' AS hearing_relevant, 'FALSE' AS request_reason, 'TRUE' AS isparent " +
        "   FROM relevantcategories p " +
        ") h " +
        "ON f.id = h.category_id " +
        "UNION " +
        "SELECT q.id, 'CATGRY' AS flag_code, q.value_en, q.category_id, '' AS categorypath, " +
        "       'FALSE' AS hearing_relevant, 'FALSE' AS request_reason, 'TRUE' AS isparent " +
        "  FROM flag_details q " +
