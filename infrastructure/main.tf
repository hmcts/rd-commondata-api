...
resource "azurerm_key_vault_secret" "commondata_s2s_secret" {
...
resource "azurerm_key_vault_secret" "POSTGRES-HOST" {
  ...
  value         = module.db-rd-commondata-api.host_name
  key_vault_id  = data.azurerm_key_vault.rd_key_vault.id
}

resource "azurerm_key_vault_secret" "POSTGRES-DATABASE" {
  name          = join("-", [var.component, "POSTGRES-DATABASE"])
  value         = module.db-rd-commondata-api.postgresql_database
  key_vault_id  = data.azurerm_key_vault.rd_key_vault.id
}
...
resource "azurerm_key_vault_secret" "POSTGRES-USER" {
  ...
  value         = module.db-rd-commondata-api.user_name
  key_vault_id  = data.azurerm_key_vault.rd_key_vault.id
}

resource "azurerm_key_vault_secret" "POSTGRES-PASS" {
  name          = join("-", [var.component, "POSTGRES-PASS"])
  value         = module.db-rd-commondata-api.postgresql_password
  key_vault_id  = data.azurerm_key_vault.rd_key_vault.id
}

module "db-rd-commondata-api" {
  ...
  env                 = var.env
  postgresql_user     = "dbrdcommondata"
  database_name       = "dbrdcommondata"
